/* eslint ignore */
//Reference to layout: https://www.npmjs.com/package/react-bubble-ui
...
export default function BubbleElement(props) {
  ...
  const interpolate = (actualMin, actualMax, val, targetMin, targetMax) => {
    return (
      ((val - actualMin) / (actualMax - actualMin)) * (targetMax - targetMin) + targetMin
    );
  };
  ...
  const getBubbleSize = (row, col) => {
    ...
    if (distanceFromEdge > 0 && distanceFromEdge <= options.fringeWidth) {
      ...
      const extra =
        (Math.max(0, distanceFromEdge - options.fringeWidth - options.size / 2) *
          options.gravitation) /
        10;
  ...
  return (
    ...
    >
      <div className={styles.container}>
        ...
        <div className={styles.scrollable} ref={scrollable} onScroll={handleScroll}>
          <div
          ...
          <div
            ...
            style={{
              width:
                options.size * options.numCols + options.gutter * (options.numCols - 1),
